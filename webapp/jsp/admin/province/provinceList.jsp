...
function deleteData(id){
    if (confirm('确定删除?')){
    	window.location="${base}/admin/province/provinceAction!delete.${actionExt}?id=" + id;
    }
}
function showData(id){
	var dialog=$.ligerDialog.open({ title:'查看', url: '${base}/admin//province/provinceAction!show.${actionExt}?id=' + id, height: 300, width: null, buttons: [
...
function itemclick(item){
	if(item.value=='add'){
		window.location="${base}/admin/province/provinceAction!add.${actionExt}";
	}
	if(item.value=='edit'){
        var row = manager.getSelectedRow();
        if (!row) { alert('请选择行'); return; }
            //alert(row.id);
		window.location="${base}/admin/province/provinceAction!edit.${actionExt}?id=" + row.id;
	}
	if(item.value=='delete'){
		...
		$.ligerDialog.confirm('确认删除模块 ' + row.code + ' 的信息？', function (yes) {
            if(yes==true){
            	window.location="${base}/admin/province/provinceAction!delete.${actionExt}?id=" + row.id;
            }
        });
	}
...
<form name="provinceForm" id="provinceForm" method="post" action="provinceAction.${actionExt}" >
...
<div id="searchbar">
编号：<input id="province.code" type="text" name="province.code" value="${province.code }"/>
