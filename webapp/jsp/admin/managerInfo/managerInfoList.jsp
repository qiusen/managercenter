...
function f_initGrid(){
    g = manager = $("#maingrid").ligerGrid({
		...
		{ display: '座机', name: 'tel',type: 'text' , width: '15%'},
		{ display: '手机', name: 'mobel',type: 'text' , width: '15%'},
		{ display: '性别：0、保密；1、男；2、女', name: 'sex',type: 'text' , width: '15%'},
		{ display: '联系地址', name: 'address',type: 'text' , width: '15%'},
...
function deleteData(id){
    if (confirm('确定删除?')){
    	window.location="${base}/admin/managerInfo/managerInfoAction!delete.${actionExt}?id=" + id;
    }
}
function showData(id){
	var dialog=$.ligerDialog.open({ title:'查看', url: '${base}/admin//managerInfo/managerInfoAction!show.${actionExt}?id=' + id, height: 300, width: null, buttons: [
...
function itemclick(item){
	if(item.value=='add'){
		window.location="${base}/admin/managerInfo/managerInfoAction!add.${actionExt}";
	}
	if(item.value=='edit'){
        var row = manager.getSelectedRow();
        if (!row) { $.ligerDialog.warn('请选择行'); return; }
            //alert(row.id);
		window.location="${base}/admin/managerInfo/managerInfoAction!edit.${actionExt}?id=" + row.id;
	}
	if(item.value=='delete'){
		...
		$.ligerDialog.confirm('确认删除模块 ' + row.email + ' 的信息？', function (yes) {
            if(yes==true){
            	window.location="${base}/admin/managerInfo/managerInfoAction!delete.${actionExt}?id=" + row.id;
            }
        });
	}
...
<form name="managerInfoForm" id="managerInfoForm" method="post" action="managerInfoAction.${actionExt}" >
...
<div id="searchbar">
邮箱：<input id="managerInfo.email" type="text" name="managerInfo.email" value="${managerInfo.email }"/>
