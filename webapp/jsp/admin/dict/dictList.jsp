...
function f_initGrid(){
    g = manager = $("#maingrid").ligerGrid({
		...
		{ display: '数据组', name: 'group',type: 'text' , width: '15%'},
		{ display: '值', name: 'value',type: 'text' , width: '15%'},
...
function deleteData(id){
    if (confirm('确定删除?')){
    	window.location="${base}/admin/dict/dictAction!delete.${actionExt}?id=" + id;
    }
}
function showData(id){
	var dialog=$.ligerDialog.open({ title:'查看', url: '${base}/admin//dict/dictAction!show.${actionExt}?id=' + id, height: 300, width: null, buttons: [
...
function itemclick(item){
	if(item.value=='add'){
		window.location="${base}/admin/dict/dictAction!add.${actionExt}";
	}
	if(item.value=='edit'){
        var row = manager.getSelectedRow();
        if (!row) { alert('请选择行'); return; }
            //alert(row.id);
		window.location="${base}/admin/dict/dictAction!edit.${actionExt}?id=" + row.id;
	}
	if(item.value=='delete'){
		...
		$.ligerDialog.confirm('确认删除模块 ' + row.code + ' 的信息？', function (yes) {
            if(yes==true){
            	window.location="${base}/admin/dict/dictAction!delete.${actionExt}?id=" + row.id;
            }
        });
	}
...
<form name="dictForm" id="dictForm" method="post" action="dictAction.${actionExt}" >
...
<div id="searchbar">
编码：<input id="dict.code" type="text" name="dict.code" value="${dict.code }"/>
