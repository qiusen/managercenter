...
function deleteData(id){
    if (confirm('确定删除?')){
    	window.location="${base}/admin/position/positionAction!delete.${actionExt}?id=" + id;
    }
}
function showData(id){
	var dialog=$.ligerDialog.open({ title:'查看', url: '${base}/admin//position/positionAction!show.${actionExt}?id=' + id, height: 300, width: null, buttons: [
...
function itemclick(item){
	if(item.value=='add'){
		window.location="${base}/admin/position/positionAction!add.${actionExt}";
	}
	if(item.value=='edit'){
        var row = manager.getSelectedRow();
        if (!row) { $.ligerDialog.warn('请选择行'); return; }
            //alert(row.id);
		window.location="${base}/admin/position/positionAction!edit.${actionExt}?id=" + row.id;
	}
	if(item.value=='delete'){
		...
		$.ligerDialog.confirm('确认删除模块 ' + row.name + ' 的信息？', function (yes) {
            if(yes==true){
            	window.location="${base}/admin/position/positionAction!delete.${actionExt}?id=" + row.id;
            }
        });
	}
...
<form name="positionForm" id="positionForm" method="post" action="positionAction.${actionExt}" >
...
<div id="searchbar">
名称：<input id="position.name" type="text" name="position.name" value="${position.name }"/>
